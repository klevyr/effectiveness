...
def test_process_can_generate_reports_at_end():
    with patch("efectividad.cli.load_config", return_value={
        ...
        "efectividad.cli.generate_global_report"
    ), patch("efectividad.cli.check_effectiveness"), patch(
        "efectividad.cli.generate_reports", return_value=["report.xlsx"]
    ) as mock_generate_reports, patch("efectividad.cli.generate_length_report") as mock_length_report:
